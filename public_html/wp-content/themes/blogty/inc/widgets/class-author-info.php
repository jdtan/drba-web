...
/**
 * Adds Blogty_Author_Info widget.
 */
class Blogty_Author_Info extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			'blogty_author_info_widget',
			esc_html__( 'Blogty: Author Info', 'blogty' ),
			array( 'description' => esc_html__( 'Displays author short info.', 'blogty' ) )
		);

		$this->social_networks = apply_filters(
			'blogty_author_widget_social_networks',
			array(
				'facebook',
				'twitter',
				'linkedin',
	...
	public function widget( $args, $instance ) {
		...
		$image_size = 'blogty-medium-img';
		if ( $image_border_radius ) {
			$image_size = 'blogty-square-img';
			$style     .= ' rd-visible';
		}
		...
		do_action( 'blogty_before_author_info' );
		?>
		<div class="blogty-author-info-widget <?php echo esc_attr( $wrapper_class ); ?>">
			<?php
				...
				if ( ! empty( $social_networks ) && is_array( $social_networks ) ) {
					...
					foreach ( $social_networks as $network ) {
						if ( ! empty( $instance[ $network ] ) ) {
							$svg = Blogty_SVG_Icons::get_social_link_svg( $instance[ $network ] );
							if ( $svg ) {
								?>
								<li>
		...
		do_action( 'blogty_after_author_info' );
	...
	public function form( $instance ) {
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'author_pos' ) ); ?>">
				<?php esc_attr_e( 'Author Position:', 'blogty' ); ?>
			</label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'author_desc' ) ); ?>">
				<?php esc_attr_e( 'Short Description:', 'blogty' ); ?>
			</label>
		...
		<div>
			<label for="<?php echo esc_attr( $this->get_field_id( 'author_img' ) ); ?>">
				<?php esc_attr_e( 'Author Image:', 'blogty' ); ?>
			</label>
			<?php
			...
			<div class="image-field">
				...
				<p>
					...
					<button type="button" class="upload_image_button button" data-uploader-title-txt="<?php esc_attr_e( 'Use Image', 'blogty' ); ?>" data-uploader-btn-txt="<?php esc_attr_e( 'Choose an Image', 'blogty' ); ?>">
						<?php esc_html_e( 'Upload/Add image', 'blogty' ); ?>
					</button>
					<button type="button" class="remove_image_button button" style="<?php echo esc_attr( $remove_button_style ); ?>"><?php esc_html_e( 'Remove image', 'blogty' ); ?></button>
				</p>
			</div>
		</div>
		<div>
			<p>
				...
				<label for="<?php echo esc_attr( $this->get_field_id( 'image_border_radius' ) ); ?>">
					<?php esc_html_e( 'Enable Image Border Radius', 'blogty' ); ?>
				</label>
			</p>
		</div>
		<div>
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'style' ) ); ?>">
					<span class="field-label"><?php esc_html_e( 'Author Info Style', 'blogty' ); ?></span>
				</label>
				<select id="<?php echo esc_attr( $this->get_field_id( 'style' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'style' ) ); ?>" class="widefat">
					<option value="style_1" <?php selected( 'style_1', $style ); ?>><?php esc_html_e( 'Style 1', 'blogty' ); ?></option>
					<option value="style_2" <?php selected( 'style_2', $style ); ?>><?php esc_html_e( 'Style 2', 'blogty' ); ?></option>
					<option value="style_3" <?php selected( 'style_3', $style ); ?>><?php esc_html_e( 'Style 3', 'blogty' ); ?></option>
				</select>
			</p>
		</div>
		<div>
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'social_links_color' ) ); ?>">
					<span class="field-label"><?php esc_html_e( 'Social Links Color', 'blogty' ); ?></span>
				</label>
				<select id="<?php echo esc_attr( $this->get_field_id( 'social_links_color' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'social_links_color' ) ); ?>" class="widefat">
					<option value="theme_color" <?php selected( 'theme_color', $social_links_color ); ?>><?php esc_html_e( 'Use Theme Color', 'blogty' ); ?></option>
					<option value="brand_color" <?php selected( 'brand_color', $social_links_color ); ?>><?php esc_html_e( 'Use Brand Color', 'blogty' ); ?></option>
				</select>
			</p>
		</div>
		<div>
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'social_links_style' ) ); ?>">
					<span class="field-label"><?php esc_html_e( 'Social Links Style', 'blogty' ); ?></span>
				</label>
				<select id="<?php echo esc_attr( $this->get_field_id( 'social_links_style' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'social_links_style' ) ); ?>" class="widefat">
					<?php
					$social_link_styles_arr = blogty_get_social_links_styles();
		...
		<div>
			<p>
				...
				<label for="<?php echo esc_attr( $this->get_field_id( 'inverted_block_color' ) ); ?>">
					<?php esc_html_e( 'Inverted Color', 'blogty' ); ?>
				</label>
			</p>
			<em><?php esc_html_e( 'Can be used if you have dark background and want ligter color on the text.', 'blogty' ); ?></em>
		</div>
		<?php
		$social_networks = $this->social_networks;
		if ( ! empty( $social_networks ) && is_array( $social_networks ) ) {
