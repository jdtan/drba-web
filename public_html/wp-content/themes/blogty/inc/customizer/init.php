 ...
 */
function blogty_customize_register( $wp_customize ) {
	...
	if ( isset( $wp_customize->selective_refresh ) ) {
		$wp_customize->selective_refresh->add_partial(
			...
			array(
				...
				'render_callback' => 'blogty_customize_partial_blogname',
			)
		);
		$wp_customize->selective_refresh->add_partial(
			'blogdescription',
			array(
				...
				'render_callback' => 'blogty_customize_partial_blogdescription',
			)
		);
	}

	// Some Defaults to reference.
	$theme_options_defaults = blogty_get_default_customizer_values();

	// Register custom section types.
	$wp_customize->register_section_type( 'Blogty_Section_Features_List' );
...
add_action( 'customize_register', 'blogty_customize_register' );
 ...
 * @return void
 */
function blogty_customize_partial_blogname() {
	bloginfo( 'name' );
}
 ...
 * @return void
 */
function blogty_customize_partial_blogdescription() {
 ...
 */
function blogty_customize_preview_js() {

	$min = defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ? '' : '.min';

	wp_enqueue_script( 'blogty-customizer', get_template_directory_uri() . '/assets/custom/js/customizer' . $min . '.js', array( 'customize-preview' ), _S_VERSION, true );
}
add_action( 'customize_preview_init', 'blogty_customize_preview_js' );
 ...
 * @since 1.0.0
 */
function blogty_customizer_control_scripts() {

	$min = defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ? '' : '.min';

	wp_enqueue_style( 'blogty-customizer-css', get_template_directory_uri() . '/assets/custom/css/customizer' . $min . '.css', array(), _S_VERSION );
	wp_enqueue_script( 'blogty-customizer-controls', get_template_directory_uri() . '/assets/custom/js/customizer-admin' . $min . '.js', array( 'jquery', 'jquery-ui-core', 'jquery-ui-sortable', 'customize-controls' ), _S_VERSION, true );
}
add_action( 'customize_controls_enqueue_scripts', 'blogty_customizer_control_scripts', 0 );
