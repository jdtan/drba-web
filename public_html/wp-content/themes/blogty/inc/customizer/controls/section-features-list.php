...
class Blogty_Section_Features_List extends WP_Customize_Section {
	...
	public $upsell_link       = 'https://unfoldwp.com/products/blogty/?utm_source=wp&utm_medium=customizer&utm_campaign=ft_upgrade';
	public $upsell_text       = '';
	public $button_link       = '';
	public $button_text       = '';
	public $class             = '';
	...
	public function json() {
		...
		$json['upsell_text']       = __( 'Upgrade Now', 'blogty' );
	...
	protected function render_template() {
		...
		<li id="accordion-section-{{ data.id }}" class="accordion-section control-section control-section-{{ data.type }} {{data.class}}">
			...
			<# if ( data.is_upsell_feature ) { #>
				<a href="{{ data.upsell_link }}" role="button" class="blogty-features-cta-btn button button-primary" target="_blank">{{ data.upsell_text }}</a>
			<# } else { #>
				<# if ( data.button_text && data.button_link ) { #>
					<a href="{{ data.button_link }}" role="button" class="blogty-features-cta-btn button button-primary" target="_blank">{{ data.button_text }}</a>
				<# } #>
			<# } #>
