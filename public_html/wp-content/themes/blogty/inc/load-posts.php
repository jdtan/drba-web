...
if ( ! function_exists( 'blogty_ajax_pagination' ) ) :
	/**
	 ...
	 */
	function blogty_ajax_pagination( $type ) {
		global $wp_query;
		if ( $wp_query->max_num_pages > 1 ) {

			?>
			<div class="blogty-load-posts-btn-wrapper" 
			data-page="1" 
			data-max-pages="<?php echo esc_attr( $wp_query->max_num_pages ); ?>" 
			data-load-type="<?php echo esc_attr( $type ); ?>"
			>
				<a href="#" class="blogty-ajax-load-btn text-decoration-none">
					<span class="blogty-ajax-btn-txt"><?php esc_html_e( 'Load More Posts', 'blogty' ); ?></span>
					<span class="blogty-ajax-loader">
						<?php blogty_the_theme_svg( 'arrow-repeat' ); ?>
					</span>
				</a>
			</div>
			<?php
...
if ( ! function_exists( 'blogty_load_posts' ) ) :
	/**
	 * Ajax Load posts Callback.
	 *
	 * @since 1.0.0
	 */
	function blogty_load_posts() {

		check_ajax_referer( 'blogty-load-posts-nonce', 'load_post_nonce' );
		...
		if ( $posts->have_posts() ) :
			...
			$error = new WP_Error( '500', __( 'No More Posts', 'blogty' ) );
...
add_action( 'wp_ajax_blogty_load_posts', 'blogty_load_posts' );
add_action( 'wp_ajax_nopriv_blogty_load_posts', 'blogty_load_posts' );
