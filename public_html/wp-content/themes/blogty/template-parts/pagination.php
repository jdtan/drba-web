...
?>
<div class="blogty-nav-pagination<?php echo ( $center_aligned_pagination ) ? ' center-aligned-pagination' : ''; ?>">
	<?php

	if ( 'button_click_load' == $pagination_type || 'infinite_scroll_load' == $pagination_type ) :

		blogty_ajax_pagination( $pagination_type );

	elseif ( 'numeric' == $pagination_type ) :

		$prev_text        = sprintf(
			'<span class="nav-prev-text">%s</span>',
			blogty_get_theme_svg('chevron-left')
		);
		$next_text        = sprintf(
			'<span class="nav-next-text">%s</span>',
			blogty_get_theme_svg('chevron-right')
		);
		...
		$prev_text = sprintf(
			...
			__( 'Older Articles', 'blogty' )
		);
		$next_text = sprintf(
			'%s <span class="nav-next-text title">%s</span>',
			'<span class="arrow" aria-hidden="true">&rarr;</span>',
			__( 'Newer Articles', 'blogty' )
		);
