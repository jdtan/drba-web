...
$class             = 'blogty-card-box';
$enabled_post_meta = get_theme_mod( 'archive_post_meta', array( 'author', 'date', 'comment', 'category' ) );
...
$meta_settings = array(
	...
	'show_icons'   => get_theme_mod( 'show_archive_post_meta_icon', true ),
);
...
if ( $show_read_more ) {
	...
	if ( ! $read_more_text ) {
		$read_more_text = __( 'Read More', 'blogty' );
	}
	$read_more_icon  = get_theme_mod( 'excerpt_read_more_icon' );
	$read_more_style = get_theme_mod( 'archive_read_more_style', 'style_3' );
}
	...
	<article id="post-<?php the_ID(); ?>" <?php post_class( $class ); ?>>
		...
		<div class="blogty-article-block-wrapper">
			...
			<header class="entry-header">
				...
				if ( 'post' === get_post_type() ) :
					if ( $cats_enabled && 'before_title' == $cat_position ) :
						blogty_post_categories( $cat_style, $cat_color, $cat_limit, $cat_label );
					endif;
				endif;
				?>
				<h2 class="entry-title color-accent-hover blogty-limit-lines <?php echo esc_attr( $archive_title_limit ); ?>">
					<a href="<?php the_permalink(); ?>" class="blogty-title-line"><?php the_title(); ?></a>
				</h2>
				...
				<div class="entry-image img-animate-zoom blogty-rounded-img">
					<a href="<?php the_permalink(); ?>">
						...
						<figure class="featured-media">
							<?php the_post_thumbnail( 'blogty-cover-image' ); ?>
			...
			if ( 'post' === get_post_type() ) :
				...
				if ( $tags_enabled ) :
					blogty_post_tags( $tag_style, $tag_limit, $tag_label );
				endif;
			endif;
