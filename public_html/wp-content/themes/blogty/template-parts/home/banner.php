...
if ( $banner_posts->have_posts() ) :
	...
	$enable_banner_dots    = get_theme_mod( 'enable_banner_dots', true );
	...
	$banner_post_meta_settings   = array(
		...
		'show_icons'   => get_theme_mod( 'show_banner_post_meta_icon', true ),
	);
	...
	if ( 'slider' == $banner_display_as ) :
		...
		$item_gap     = absint( get_theme_mod( 'banner_carousel_item_gap', 24 ) );
		if ( $item_gap == 0 ) {
			$img_class = '';
		}
	...
	?>
	<section class="blogty-section-banner-wrapper <?php echo esc_attr( $wrapper_class ); ?>">
		<div class="blogty-section-banner<?php echo esc_attr( $container_class ); ?>">
			<div class="row<?php echo esc_attr( 'full-width' == $banner_layout ) ? ' gy-0' : ' g-4'; ?>">
				<div class="<?php echo esc_attr( $slider_wrapper_class ); ?>">
					...
					<?php endif; ?>
					<div class="blogty-banner-wrapper blogty-swiper-inner-bullets swiper" data-banner='<?php echo esc_attr( json_encode( $data_banner ) ); ?>'>
						<div class="blogty-banner swiper-wrapper">
							<?php
										...
										<div class="banner-block-wrapper img-animate-zoom">
											<div class="banner-image <?php echo esc_attr( $img_class ); ?>">
												<a href="<?php the_permalink(); ?>">
													<?php if ( $enable_banner_overlay ) : ?>
														<span aria-hidden="true" class="blogty-block-overlay" style="<?php echo $banner_overlay_style; ?>"></span>
													<?php endif; ?>
													<?php
													the_post_thumbnail(
														$image_size,
											...
											<div class="banner-caption">
												<div class="banner-caption-inner">
													<?php
													if ( $show_banner_category ) :
														blogty_post_categories( $banner_cat_style, $banner_cat_color, $banner_cat_limit );
													endif;
													?>
													<h3 class="banner-title no-margin blogty-limit-lines <?php echo esc_attr( $title_limit ); ?>">
														<a href="<?php the_permalink(); ?>" class="text-decoration-none blogty-title-line">
															<?php the_title(); ?>
														</a>
													</h3>
													<?php blogty_post_meta_info( $banner_post_meta, $banner_post_meta_settings ); ?>
													<?php
													...
													if ( $enable_banner_read_more_btn ) {
														...
														<div class="banner-read-more">
															<a href="<?php the_permalink(); ?>" class="blogty-btn-link text-decoration-none <?php echo esc_attr( $banner_read_more_style ); ?>">
																<?php
																...
																if ( $banner_read_more_icon ) {
																	?>
																	<span><?php blogty_the_theme_svg( $banner_read_more_icon ); ?></span>
																	<?php
																}
																?>
															</a>
